...
public class ReleaseFragment extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        @SuppressLint("InflateParams") ViewGroup view = (ViewGroup) inflater.inflate(R.layout.fragment_release, null);
        ...
        ViewPager pagerView = (ViewPager) view.findViewById(R.id.pager);
        ...
        PagerAdapter pagerAdapter = new ScreenSlidePagerAdapter(this.getChildFragmentManager(), tomes);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == favoriteRelButton.getId()) {
            ...
            int id = (int) SqLiteHelper.getInstance(getContext()).createManga(mg);
            // New manga created -> add to the favorite list
            if (id != 0) {
                SqLiteHelper.getInstance(getContext()).createFavorite(id);
                ...
                int tmp = SqLiteHelper.getInstance(getContext()).getManga_id(mg.getTitle());
                if (SqLiteHelper.getInstance(getContext()).isFollow(tmp) == 0) { // manga was not followed
                    // followed now and add the others tomes if there are.
                    SqLiteHelper.getInstance(getContext()).updateFavorite(tmp, 1);
                    this.AddTomeOfFavoriteManga(mg, id);
                } else { // manga was followed
                    // stop the following of the manga
                    SqLiteHelper.getInstance(getContext()).updateFavorite(tmp, 0);
                }
            }
    ...
    private void AddTomeOfFavoriteManga(MangaClass mg, int id) {
        for (int i = 0; i < tomes.size(); i++) {
            if (tomes.get(i).getTitleManga().compareTo(mg.getTitle()) == 0) {
                ...
                SqLiteHelper.getInstance(getContext()).createTome(tome, id);
            }
        }
    }
    ...
    private class PageListener extends ViewPager.SimpleOnPageChangeListener {
        ...
        private ArrayList<TomeClass> arrayView;
        private ImageButton favoriteRelButton;
        private SqLiteHelper db;
        ...
        public void onPageSelected(int position) {
            ...
            int id = SqLiteHelper.getInstance(context).getManga_id(arrayView.get(currentPage).getTitleManga());
            int isFollow = SqLiteHelper.getInstance(context).isFollow(id);
            if (isFollow == 1) {
                favoriteRelButton.setBackgroundResource(R.drawable.yellowstar);
                favoriteRelButton.setTag(R.drawable.yellowstar);
            } else {
